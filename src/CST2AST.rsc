...
AQuestion cst2ast(Question q) {
  switch (q) {
    case (Question) `<Str s> <Id i>: <Type t>`:
      return question(label("<s>", src=s@\loc), id("<i>", src=i@\loc), cst2ast(t), src=q@\loc);
    case (Question) `<Str s> <Id i>: <Type t> = <Expr e>`:
      return computed_question(label("<s>", src=s@\loc), id("<i>", src=i@\loc), cst2ast(t), cst2ast(e), src=q@\loc);
